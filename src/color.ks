...
let $caster = {
	alpha(n = null, percentage = false): Number { // {{{
		const i = parseFloat(n)

		return 1 if i == NaN else (percentage ? i / 100 : i).limit(0, 1).round(3)
	} // }}}
	ff(n): Number { // {{{
		return parseFloat(n).limit(0, 255).round()
	} // }}}
	percentage(n): Number { // {{{
		return parseFloat(n).limit(0, 100).round(1)
	} // }}}
}
	...
	srgb(that: Color, args: array): bool { // {{{
		if args.length == 1 {
			if args[0] is number {
				...
				let color = (args[0] as string).toLowerCase().replace(/[^a-z0-9,.()#%]/g, '')
				...
				if match ?= /^#?([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/.exec(color) {
					that._space = Space::SRGB
					that._red = parseInt(match[1], 16)
					that._green = parseInt(match[2], 16)
					that._blue = parseInt(match[3], 16)
					that._alpha = $caster.alpha(parseInt(match[4], 16) / 255)
					return true
				}
				// #ff9000, #ff0000
				...
				// #fffa
					...
					that._space = Space::SRGB
					that._red = parseInt(match[1] + match[1], 16)
					that._green = parseInt(match[2] + match[2], 16)
					that._blue = parseInt(match[3] + match[3], 16)
					that._alpha = $caster.alpha(parseInt(match[4] + match[4], 16) / 255)
					return true
				}
				// #f00, #fff
...
export class Color {
	...
	blend(color: Color, percentage: Number, space: Space = Space::SRGB, alpha: bool = false): Color ~ Error { // {{{
		if alpha {
	...
	#[error(off)]
	gradient(endColor: Color, length: Number): array<Color> { // {{{
